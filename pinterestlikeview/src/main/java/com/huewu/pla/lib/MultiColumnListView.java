...
public class MultiColumnListView extends PLA_ListView {
	...
	private ArrayList<Integer> mHeaders = new ArrayList<Integer>();
	private int mColumnPaddingLeft = 0;
	private int mColumnPaddingRight = 0;
	
	private int mColumnSelectorPadding = 8;
	private int mHeadersCount = 0;
	
	protected ColumnWidthSetListener mColWidthListener;
...
//		int width = (getMeasuredWidth() - mListPadding.left - mListPadding.right) / mColumnNumber;
		...
		if(mColWidthListener != null){
			mColWidthListener.onColumnWidthSet(width);
		}
	...
	protected void onMeasureChild(View child, int position, int widthMeasureSpec, int heightMeasureSpec) {
		if(isFixedView(child))
			...
			int colWidth = getColumnWidth(position);
			int padding = child.getPaddingRight();
			if(mColWidthListener != null){
				mColWidthListener.onChildPaddingSet(padding);
			}
	...
	private int calculateColumnIndex(int position){
		return (position + 1) % mColumnNumber;
	}
	...
	protected void onItemAddedToList(int position, boolean flow ) {
		...
		if( isHeaderOrFooterPosition(position) == false){
			...
			int index = calculateColumnIndex(position);
			mItems.append(position, col.getIndex());
		}else{
			if(position < 8){//maximum 8 headers allowed
				boolean contains = false;
				for(int i = 0; i < mHeaders.size(); i++){
					if(position == mHeaders.get(i)){
						contains = true;
					}
				}
				if(!contains){
					mHeaders.add(position);
					mHeadersCount++;
				}
			}
		}
	...
	private Column getNextColumn(boolean flow, int position) {
		...
		if( lastVisiblePos < mColumnNumber )
			return mColumns[Math.max(0, lastVisiblePos - mHeadersCount)];
	...
	public void setColumnWidthSetListener(ColumnWidthSetListener listener){
		mColWidthListener = listener;
	}
	...
	public void setColumnNumber(int columnNumber) {
	    ...
	    for( int i = 0; i < mColumnNumber; ++i )
	     mColumns[i] = new Column(i);
	    mFixedColumn = new FixedColumn();
	    mItems = new SparseIntArray();
	}
	
	public interface ColumnWidthSetListener{
		public void onColumnWidthSet(int width);
		public void onChildPaddingSet(int padding);
	}
